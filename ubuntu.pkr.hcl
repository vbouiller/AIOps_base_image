...
local "vault_ca" {
  expression = vault("/ssh-client-signer/config/ca", "public_key")
  sensitive  = true
}
...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "apt-get upgrade -y",
      "echo '${local.vault_ca}' > /etc/ssh/trusted-user-ca-keys.pem", # Setting up SSH connection details via Vault SSH Secret Engine CA
